...
const COMPARE_KEYS: { key: string; label: string; format?: (v: number | string) => string; best?: 'min' | 'max' }[] = [
  ...
  { key: 'avgRating', label: 'Rating', format: (v) => `${v}★ (${Number(v) > 0 ? 'rated' : 'no reviews'})`, best: 'max' },
...
export default function ComparePropertiesPage() {
  ...
  const getBestIndex = (key: string, best: 'min' | 'max'): number => {
    const values = properties.map((p) => Number((p as unknown as Record<string, unknown>)[key]));
                ...
                return (
                  <tr key={key} className="border-t border-gray-100">
                    ...
                    {properties.map((p, idx) => {
                      const rawVal = (p as unknown as Record<string, number | string>)[key];
