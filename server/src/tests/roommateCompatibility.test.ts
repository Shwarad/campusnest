...
function makeProfile(overrides: Partial<IRoommateProfile> = {}): IRoommateProfile {
  return {
...
describe('Roommate Compatibility Algorithm', () => {
  ...
  it('should give partial budget overlap score', () => {
    const p1 = makeProfile({ budget: { min: 4000, max: 8000 } });
    const p2 = makeProfile({ budget: { min: 7000, max: 12000 } });
  ...
  it('should return 0 budget score for non-overlapping budgets', () => {
    const p1 = makeProfile({ budget: { min: 2000, max: 3000 } });
    const p2 = makeProfile({ budget: { min: 8000, max: 12000 } });
    const r = calculateRoommateCompatibility(p1, p2);
    expect(r.breakdown.budget).toBe(0);
  });

  it('should identify strong matches correctly', () => {
    const p1 = makeProfile({ budget: { min: 4000, max: 8000 }, preferredLocality: 'Jalukbari', cleanliness: 'very_clean', sleepSchedule: 'early_bird' });
    const p2 = makeProfile({ budget: { min: 4000, max: 8000 }, preferredLocality: 'Jalukbari', cleanliness: 'very_clean', sleepSchedule: 'early_bird' });
    const r = calculateRoommateCompatibility(p1, p2);
  ...
  it('should return score between 0 and 100', () => {
    const worst = makeProfile({ smoking: true, drinking: true, sleepSchedule: 'early_bird', cleanliness: 'very_clean', budget: { min: 1000, max: 2000 } });
    const best = makeProfile({ smoking: false, drinking: false, sleepSchedule: 'night_owl', cleanliness: 'relaxed', budget: { min: 8000, max: 15000 } });
