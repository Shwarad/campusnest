...
export function calculateRoommateCompatibility(
  profile1: RoommateProfileForCompatibility,
  profile2: RoommateProfileForCompatibility
): CompatibilityResult {
  const breakdown: Record<string, number> = {};

  // Normalise budget: support both {budgetMin/budgetMax} and legacy {budget:{min,max}}
  const b1min = profile1.budgetMin ?? profile1.budget?.min ?? 0;
  const b1max = profile1.budgetMax ?? profile1.budget?.max ?? 0;
  const b2min = profile2.budgetMin ?? profile2.budget?.min ?? 0;
  const b2max = profile2.budgetMax ?? profile2.budget?.max ?? 0;

  // Budget compatibility (20%)
  const budgetOverlap = calculateBudgetOverlap({ min: b1min, max: b1max }, { min: b2min, max: b2max });
